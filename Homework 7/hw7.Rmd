...
getNextGenMutation <- function(p, popsize, u, v) {
  psi <- p - p*popsize*u + (1-p)*popsize*v # adjust p by mutation rate
  return(rbinom(length(p), popsize, psi)/popsize)
}
...
```{r}
# tidy the data a bit... makes for sexy tables later... an unnecessary number
# of cpu cycles died unnecessarily to give us this information...

header <- paste('T', seq(1,ngenerations), sep='')

colnames(wf_m1) <- header
wf_m1 <- tbl_df(wf_m1)
# wf_m1_m <- gather(tbl_df(wf_m1), Generation, p) %>% 
#   mutate(AdjMutRate=as.character(4*N*u1))

colnames(wf_m2) <- header
wf_m2 <- tbl_df(wf_m2)
# wf_m2_m <- gather(tbl_df(wf_m2), Generation, p) %>% 
#   mutate(AdjMutRate=as.character(4*N*u2))

colnames(wf_m3) <- header
wf_m3 <- tbl_df(wf_m3)
# wf_m3_m <- gather(tbl_df(wf_m3), Generation, p) %>% 
#   mutate(AdjMutRate=as.character(4*N*u3))

# df1_m <- gather(tbl_df(wf_m1), Generation, 'AdjMutRate=2')
# df2_m <- gather(tbl_df(wf_m2), Generation, 'AdjMutRate=1')
# df3_m <- gather(tbl_df(wf_m3), Generation, 'AdjMutRate=.5')
# df_m <- bind_cols(df1_m, select(df2_m, 'AdjMutRate=1'), select(df3_m, 'AdjMutRate=.5')) %>%
#   gather(AdjMutRate, p, -Generation)
```
...
wf_m1 %>% select(T1, T500) %>%
  gather(Generation, p) %>%
  ggplot() + aes(x=p) + facet_grid(.~Generation, scales='free') + geom_histogram(bins=50)
